...
**当前焦点**: MarkdownBackend eval 对比 + RL 训练端到端验证

**最大差距**: RL 训练从未跑通（0 checkpoints）
...
- 340 tests, 9 simulation strategies ALL PASS（v0.6.7）
- 46 real evaluations, 5 models — Qwen3.5=30%, Kimi=28%, Qwen3-235B=18%, MiniMax=13%, GLM-5=8%
...
**46 次真实评测，5 个模型，5 个厂商。** 多模型汇总：

| 模型 | N | Composite | Breadth | Maint. | Reasoning | Efficiency |
|------|---|-----------|---------|--------|-----------|------------|
| Qwen3.5-397B | 12 | **21%** | 12% | 49% | 12% | 13% |
| Kimi-K2.5 | 18 | **18%** | 13% | 37% | 10% | 11% |
| Qwen3-235B | 7 | **19%** | 16% | 48% | 2% | 11% |
| MiniMax-M2.5 | 7 | **10%** | 3% | 27% | 4% | 5% |
| GLM-5 | 2 | **0%** | 0% | 0% | 0% | 0% |

**关键发现**（A62 数据分析）：
- Retrieval 11% 正确率是最大瓶颈（60% 弃权 = 存了但搜不到）
- 7 个推理类型 0%（outlier/comparison/cross_category 等）——系统性模型能力差距
- Maintenance 最强轴（27-49%），Efficiency 最弱（5-13%）
- 三强模型统计等价（18-21%），MiniMax 明显弱（10%），GLM-5 工具调用完全失败
...
├── 46 JSON results + trajectory files
