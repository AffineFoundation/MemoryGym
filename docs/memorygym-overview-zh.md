# MemoryGym：LLM Agent 记忆管理能力评测系统
...
**核心空白**：目前没有评测系统专门测试 Agent 在**写入时**的记忆管理决策质量。MemoryGym 填补这个空白。

## 二、核心价值

### 1. 测试真实能力，而非可以 hack 的指标

MemoryGym 的设计核心是**反作弊**。我们用 6 种对抗策略验证评测不可作弊：
...
python -m memorygym.bench --seeds 30 --validate -o results.json

# 指定模板和策略
python -m memorygym.bench --seed 0 --template company --strategy strategic guesser -v

# 反指纹测试
python -m memorygym.bench --seed 0 --eval-salt 42 --validate
```
...
python -m memorygym.bench --seed 0 --model gpt-4o -v

# 使用 Chutes API
export CHUTES_API_KEY=cpk-...
python -m memorygym.bench --seed 0 --model deepseek-ai/DeepSeek-V3 \
...
```bash
inspect eval memorygym/worlds/eval_task.py \
    -M openai/gpt-4o -T seed=42 -T template=company
```

## 六、与竞品对比

| 特性 | MemoryGym | Needle-in-Haystack | RAG 评测 | LongBench |
|------|-------------|-------------------|---------|-----------|
| 测试存储决策 | ✓ | ✗ | ✗ | ✗ |
| 测试记忆更新 | ✓ | ✗ | ✗ | ✗ |
| 写入预算限制 | ✓ | N/A | N/A | N/A |
