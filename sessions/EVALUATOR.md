...
### 批次 13 — v3 基线重跑（Phase 60 bug 修复后）

**背景**：批次 12 因 Bug 2（修正消息旧工具名）和 Bug 4（JSON 保存 crash）导致数据不完整。Phase 60 已修复全部 6 个 bug（v0.6.3）。重跑以获得完整 v3 基线。

**任务**：2 模型 × 3 模板 × seed 0，共 6 次评测（与批次 12 相同配置）。
...
**重点关注**：
- Corrections 是否 >0（Bug 2 修复后模型应能使用 Edit）
