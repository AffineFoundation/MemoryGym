...
**目的**：GLM-5 和 MiniMax 只有 2 和 7 个 eval 数据点，覆盖不足。扩展低数据量模型的 eval 覆盖。

**任务**：2 模型 × 3 模板 × seed 1，共 6 次评测。

```bash
python -m memorygym.bench --model zai-org/GLM-5-TEE --seed 1 --template company
python -m memorygym.bench --model zai-org/GLM-5-TEE --seed 1 --template research
python -m memorygym.bench --model zai-org/GLM-5-TEE --seed 1 --template hospital
