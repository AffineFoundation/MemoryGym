...
### 批次 9 — mem0 后端评测 ⛔ 等待执行者确认

**前置条件**：执行者在 Phase 52 中已自行跑通 `--backend mem0` 冒烟评测（seed 0, company）。**在 Phase 52 标记完成之前不要尝试**。

Phase 52 完成后，执行者已验证 company 模板跑通。评测线程负责扩展到其他模板：

```bash
# 执行者已跑通 company，评测线程补充其他模板
...
**注意**：mem0 分数与 ChromaDB **不可直接比较**（存储粒度不同）。目标是验证多模板覆盖，不是对比分数。

**记录要点**：
- 搜索精度如何？（mem0 有自己的 LLM-based search）
- 与 ChromaDB 同模型/同 seed/同模板的分数对比（仅供参考）

### 批次 10 — 跨 tier 多 seed 验证

批次 6-8 发现 hard(24%) > standard(12%)，反直觉。需要更多数据排除单 seed 偶然性。

```bash
# standard tier 多 seed（验证 standard 是否真的比 hard 低）
python -m memorygym.bench --model moonshotai/Kimi-K2.5-TEE --seed 1 --template company --tier standard
python -m memorygym.bench --model moonshotai/Kimi-K2.5-TEE --seed 2 --template company --tier standard

# hard tier 多 seed
python -m memorygym.bench --model moonshotai/Kimi-K2.5-TEE --seed 1 --template company --tier hard
python -m memorygym.bench --model moonshotai/Kimi-K2.5-TEE --seed 2 --template company --tier hard
```

目标：每个 tier 至少 3 个 seed，确认 tier 间分数关系是否稳定。
