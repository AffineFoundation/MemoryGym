...
### 批次 17 — 多模板 v0.8.x 基线（Phase 79-85 修复后）

**目的**：Phase 79-85 修复了多个 bug（write counting, backend cleanup, tool names, defaults）。需要 hospital/sport/research 的 v0.8.x 数据来建立跨模板基线。Company 已有 8 seeds（s0-s7），其他模板仅有旧版本数据。

**任务**：Qwen3.5 × 3 模板 × seed 0，共 3 次评测。
...
**重点关注**：
- hospital maintenance 轴（template correction_rate=0.15 最高）
- sport 与 company 的 composite 对比
- 跨模板分数稳定性
