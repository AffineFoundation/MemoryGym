...
### 批次 12 — v3 基线（OpenClaw 工具接口 + 提示词中立化）

**背景**：Phase 57-59 完成后系统发生重大变更：(1) 系统提示词中立化，(2) mem0 移除，(3) 工具接口从 memory_store/forget 改为 Write/Edit/Read。这是 v3 数据，与 v2 不可比较。需要建立 v3 基线。

**前置条件**：等 Phase 60 完成（代码审查 + 剩余 Step 完成）后再跑。确认 `python -m memorygym.bench --seeds 3 --validate` 通过。

**任务**：2 个模型 × 3 个模板 × seed 0，共 6 次评测。
...
**重点关注**：
- Write/Edit 工具是否被正确调用（检查 eval JSON 的 conversation）
- 与 v2 批次 1/3 的定性对比（不直接比数值，看模式变化）
- 模型是否能自主发现 Edit 用于更新的用法（系统提示词不再提示策略）
